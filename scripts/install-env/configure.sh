...
DB_USER="${DB_USER:-users_flow}"
DB_PASS="${DB_PASS:-a1234567}"
...
DATABASE_URL="mysql://users_flow:a1234567@127.0.0.1:3306/users_manager"
...
echo "  首次 apply-dist 会自动跑 prisma/seed.js（按上述账号建管理员；已存在则不改密码）"
echo "  HD_MNEMONIC 勿再改；请离线备份。"
echo "下一步："
echo "  · 上传 dist 后 sudo bash ${SCRIPT_DIR}/apply-dist.sh"
echo "  · 日常启停 → sudo bash ${SCRIPT_DIR}/../deploy/run/start.sh | restart.sh | stop.sh"
