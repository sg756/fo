...
import { AdminApi, getAdminMe, getRememberedCredentials, setRememberedCredentials } from '../api';
...
export function AdminsPage() {
  ...
  const me = getAdminMe();
  const canManageOthers = !!me && (!me.adminRole || me.adminRole.code === 'system');
  const pager = usePager(20);

  function canChangePassword(adminId: string) {
    return canManageOthers || adminId === me?.id;
  }
  ...
  return (
    <div className="page-list">
          ...
          <tbody>
            {items
              ...
              <tr key={a.id}>
                ...
                <td>
                  {canChangePassword(a.id) ? (
                    <button className="ghost" disabled={!!busy} onClick={() => setPwdAdminId(a.id)}>
                      改密码
                    </button>
                  ) : null}
                  {a.status === 'ACTIVE' ? (
      ...
      <UserPasswordModal
        userId={pwdAdminId}
        title={pwdAdminId === me?.id ? '修改我的密码' : '修改管理员密码'}
        onClose={() => setPwdAdminId(null)}
        onSubmit={async (id, password) => {
          await AdminApi.setAdminPassword(id, password);
          if (id === me?.id) {
            const cred = getRememberedCredentials();
            if (cred) setRememberedCredentials(cred.email, password);
          }
        }}
      />
    </div>
